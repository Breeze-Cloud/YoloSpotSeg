...
class SegmentationValidator(DetectionValidator):
    ...
    def update_metrics(self, preds, batch):
        ...
        for si, (pred, proto) in enumerate(zip(preds[0], preds[1])):
            ...
            if npr == 0:
                # image_name = os.path.basename(batch['im_file'][si])
                # self.per_image_stats[image_name] = None
                if nl:
            ...
            # singImgStats = [(correct_masks, correct_bboxes, pred[:, 4], pred[:, 5], cls.squeeze(-1))]
            # newSingleImgStats = [torch.cat(x, 0).cpu().numpy() for x in zip(*singImgStats)]
            # self.metrics.process(*newSingleImgStats)
            # image_name = os.path.basename(['im_file'][si])
            # self.per_image_stats[image_name] = self.metrics.seg.f1

            # self.single_stats = [(correct_masks, correct_bboxes, pred[:, 4], pred[:, 5], cls.squeeze(-1))]
            # imgStats = [torch.cat(x, 0).cpu().numpy() for x in zip(*self.single_stats)]
            # self.metrics.process(*imgStats)
            # image_name = os.path.basename(batch['im_file'][si])
            # self.per_image_stats[image_name] = self.metrics.seg.f1
