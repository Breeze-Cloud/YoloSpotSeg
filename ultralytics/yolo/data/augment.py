    ...
    def _mix_transform(self, labels):
        ...
        for i in range(4):
            ...
            if i == 0:  # top left
                ...
                img4 = np.full((s * 2, s * 2, img.shape[2]), 114, dtype=img.dtype)
...
class GradientSharp:
    ...
    def __call__(self, labels):
        ...
        if img.dtype == np.uint8:
            ...
            im = im + img + gradxy_remove_noise
            img = np.clip(im, min, max).astype(np.uint8)

        if len(img.shape) == 2:
            img = np.repeat(np.expand_dims(img, axis=-1), 3, axis=2)
